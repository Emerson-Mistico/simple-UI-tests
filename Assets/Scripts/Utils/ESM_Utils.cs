...
#if UNITY_EDITOR
    #region MENU ITENS
    [MenuItem("ESM UTILS/Cria um CUBO %,", false, 100000)]

    public static void spawnCube()
    {
        spawnItemByType(PrimitiveType.Cube);
    }

    [MenuItem("ESM UTILS/Cria uma ESFERA %M", false, 100000)]

    public static void spawnSphere()
    {
        spawnItemByType(PrimitiveType.Sphere);
    }
